...
function createMeetings() {
	...
	if (nOfMeetings == 0) { 
		$("#no_meetings").text("Il n'y a pas de salles actives en ce moment. Mais vous pouvez en créer une!");
		$("#meetings").text('');
	}
	else 
		$("#no_meetings").text('');
...
function updateMeetingInfo() {
	$.ajax({
		...
		success: function(xml) {
			...
			//alert(JSON.stringify(meetings, null, 4));
			initializeDivTags();
			createMeetings();
		},
		error: function() {
...
function createMeetingTable(meeting) {	
	...
	var tableContent = '<table name="' + meeting.meetingID + '" class="tableau" cellspacing="0" summary="The current participants in a meeting"><caption>' + meeting.meetingID + '<caption><tr><th scope="col" abbr="Name">Nom</th><th scope="col" abbr="Role">Role</th>';
	...
	if (meeting.attendees.attendee) {
		for (var i in meeting.attendees.attendee) {
			...
			tableContent += '<tr id="' + tableRowId + '"><td>' + attendee.fullName + '</td><td>' + attendee.role + '</td></tr>';
	...
	tableContent +='<td colspan="2"><a href="'+meeting.test+'" class="button">Rejoindre</a><td>';
	tableContent += '</table>';
