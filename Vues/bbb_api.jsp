	...
	try {	
		...
		for (int i = 0; i < meetingsList.getLength(); i++) {
			...
			if (data.indexOf("<response>") != -1) {
				...
				//Remplacer les mots anglais par du français
				data = data.replaceFirst("<role>MODERATOR</role>","<role>Modérateur</role>");
				data = data.replaceFirst("<role>VIEWER</role>","<role>Utilisateur</role>");
				
				int startIndex = data.indexOf(startResponse) + startResponse.length();
				int endIndex = data.indexOf(endResponse);

				newXMldocument +=  "<meeting>" + data.substring(startIndex, endIndex) + "<test>" + inviteURL+"</test></meeting>";
